...
declare -a SAMPLES=("sample0001A" "sample0002A" "sample0003A" "sample0004A" "sample0005A" "sample0006A" "sample0007A" "sample0008A" "sample0009A" "sample0010A" "sample0011A")
JOBID_PREFIX=""
...
${CALL_JSUB} -s --protocol ${PROTOCOL_FILE} --vars ${VARS_FILE} --fvars ${FVARS_FILE} --summary-prefix ${SUMMARY_PREFIX} --prefix-completed ${COMPLETED_PREFIX} --prefix-incomplete ${INCOMPLETE_PREFIX} 
...
${CALL_JSUB} -j -u ${GENERATED_SUMMARY_LIST} $(getCommonHeaderOptionString "$JOB_HEADER") --job-prefix ${JOB_PREFIX} -e "lsf_output/lsf_" --prefix-completed ${COMPLETED_PREFIX} --prefix-incomplete ${INCOMPLETE_PREFIX} 
...
${CALL_JSUB} -b -o ${GENERATED_JOB_LIST} --prefix-completed ${COMPLETED_PREFIX} --prefix-incomplete ${INCOMPLETE_PREFIX} 
bjobs
idx=0
for sample in "${SAMPLES[@]}"; do
