...
EXPECTED_SUMMARY_LIST="../expected_files/fvars__fvars.list-summaries"
...
EXPECTED_JOB_LIST="../expected/fvars.list-jobs"
...
GENERATED_SUBMITTED_JOBS_LIST="fvars.list-jobs.submitted"
...
. ../assert.sh
echo ""
echo "Running integration test: fvars..."
...
${CALL_JSUB} -sj -p ${PROTOCOL_FILE} --fvars ${FVARS_FILE}
# Check that a summary file and a summary listing file are generated from the protocol
assert "file_exists ${GENERATED_SUMMARY_01}" "yes"
assert "file_exists ${GENERATED_SUMMARY_02}" "yes"
assert "file_exists ${GENERATED_SUMMARY_03}" "yes"
