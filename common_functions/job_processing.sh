...
function check_completion { # Check that the input file ends a string indicating successful job completion
  if [[ $(tail -n1 "$1") != "$JSUB_SUCCESSFUL_COMPLETION"* ]]; then 
    echo "$dateTime ""$JSUB_JOB_ID"" Terminating job.  Did not find the string \"""$JSUB_SUCCESSFUL_COMPLETION""\", indicating successful completion of parent job, at the end of the file: ""$1"""
