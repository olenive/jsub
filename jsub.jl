...
@add_arg_table argSettings begin
  ...
  "-s", "--summary"
    action = :store_true
    help = "Generate summary files from protocol and .vars, .fvars and list files (where supplied).  Stops after generating summary files."
...
flagSummary = parsed_args["summary"];
...
flagVerbose && println("# Read .protocol file (of 1 column ) and expand variables from .vars");
